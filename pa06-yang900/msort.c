...
void msort(Student * base, int nel, int (*compar)(const void *, const void *)) {
  ...
  msort(&base[0], mid, compar); 
	...
	//2b. Sortthe second half of the array. Pass in the address of the 
...
Student * merge(Student * base1, int nel1, Student * base2, int nel2, int (*compar)(const void *, const void *)) {
	...
	//FILL IN
  Student * mergeList = malloc(sizeof(Student) * (nel1 + nel2));
  ...
  int result = 0;
	...
	//FILL IN
  for(cursor3 = 0; cursor3 < nel1 + nel2; cursor3++)
    {
      if(cursor1 < nel1 && cursor2 < nel2)
	{
	  result = compar(&base1[cursor1], &base2[cursor2]); 
	  if(result > 0)
	    {
	      mergeList[cursor3] = base2[cursor2];
	      cursor2 ++;
	    }
	  else  if(result <= 0)
	    {
	      mergeList[cursor3] = base1[cursor1];
	      cursor1 ++;
	    } 
	}
      else if(cursor1 < nel1)
	{ 
	  mergeList[cursor3] = base1[cursor1];
	  cursor1 ++;
	}
